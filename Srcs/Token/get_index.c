...
/*   Updated: 2024/05/19 16:29:47 by rmohamma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*type = curr_type;
	if (curr_type == HEREDOC || curr_type == APPEND)
		i = i + 1;
	else if (curr_type == WORD)
	{
		...
		{
			if (line[i] == '\"')
				i = get_next_quote_index(line, i + 1, D_QUOT);
			else if (line[i] == '\'')
				i = get_next_quote_index(line, i + 1, S_QUOT);
